...
async def main() -> None:
    parser = argparse.ArgumentParser(
        description="Test Schwab Level 1 streaming (SPY by default)"
    )
    parser.add_argument(
        "--user-id",
        required=False,
    ...
    if not user_id or user_id == "<PUT_USER_ID_HERE>":
        print(
            "Please set DEFAULT_USER_ID in scripts/test_stream_l1.py or provide --user-id."
        )
        return
    ...
    def on_level_one_equity(msg):
        print("L1 message:", msg)

    stream.add_level_one_equity_handler(on_level_one_equity)

    # Open websocket session
    await stream.login()

    print(f"Subscribing to level one quotes for: {symbols} ...")
    ...
    try:
        ...
        print("Unsubscribing...")
        try:
            await stream.level_one_equity_unsubs(symbols)
        except Exception:
            pass
        try:
            task.cancel()
